//! End-to-end tests for the GDS trust engine. Requires the docker stack
//! (Neo4j with the GDS plugin + Redis) to be up.
//!
//! Every `trust` assertion from a recompute lives in the one run-the-engine test
//! below on purpose: a run writes `trust` across the whole `:User` label, so two
//! concurrent runs clobber each other. One test means no nextest serialization
//! to configure.
...
use nexusd::trust::neo4j::queries::{
