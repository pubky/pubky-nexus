...
use nexus_watcher::events::retry::{InitialBackoff, RetryEvent, RetryScheduler, RetryStore};
...
async fn test_batch_continues_after_single_failure() -> Result<()> {
    ...
    assert!(
        store
            .get(&RetryEvent::index_key(&first_uri))
            .await?
            .is_some(),
    ...
    assert!(
        store
            .get(&RetryEvent::index_key(&second_uri))
            .await?
            .is_some(),
