...
async fn test_multi_hs_event_processing_with_homeserver_limit() -> Result<()> {
    ...
    // run_all excludes the default HS, so 4 non-default HSs available, limited to 3
    let stats = runner.run_all().await.unwrap().0;
...
async fn test_multi_hs_event_processing_with_homeserver_limit_one() -> Result<()> {
    ...
    // (the default homeserver is now excluded from run_all)
    ...
    assert_ne!(
        ...
        "Default homeserver should be excluded from pre_run_all"
    );
...
async fn test_multi_hs_event_processing_with_timeout() -> Result<()> {
    ...
    // run_all excludes the default HS (0s sleep), so only index 1 and 2 are processed.
...
async fn test_multi_hs_event_processing_with_panic() -> Result<()> {
    ...
    // run_all excludes the default HS (first success), so 2 success + 2 panic are processed
    let stats = runner.run_all().await.unwrap().0;
