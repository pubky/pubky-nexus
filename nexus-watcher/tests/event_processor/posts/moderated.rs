use crate::{
    event_processor::posts::utils::find_post_details,
    event_processor::utils::WatcherTest,
};
use anyhow::Result;
use chrono::Utc;
...
async fn test_moderated_post_lifecycle() -> Result<()> {
    ...
    let moderator_recovery_file = fs::read("./tests/event_processor/utils/moderator_key.pkarr").await.unwrap();
    let moderator_key =
