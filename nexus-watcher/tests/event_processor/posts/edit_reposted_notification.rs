use crate::event_processor::utils::test_ids::posts::edit_reposted_notification as ids;
...
async fn test_edit_reposted_post_notification() -> Result<()> {
    ...
    let (repost_id, _repost_path) = test.create_post(&user_b_kp, &repost, ids::REPOST).await?;
