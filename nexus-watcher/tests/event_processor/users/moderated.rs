...
use crate::{event_processor::users::utils::find_user_details, event_processor::utils::WatcherTest};
use anyhow::Result;
use chrono::Utc;
...
async fn test_moderated_user_lifecycle() -> Result<()> {
    ...
    let mod_file = fs::read("./tests/event_processor/utils/moderator_key.pkarr").await.unwrap();
