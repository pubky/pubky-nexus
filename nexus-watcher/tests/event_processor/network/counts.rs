...
use crate::event_processor::utils::WatcherTest;
use anyhow::Result;
use nexus_common::{
...
async fn test_large_network_scenario_counts() -> Result<()> {
    ...
    if !PROCESS_EVENTS_ONE_BY_ONE {
        for _ in 1..=100 {
            let cursor = test.event_processor.run(shutdown_rx.clone()).await.unwrap();
            test.event_processor.homeserver.cursor = cursor;
        }
    }
