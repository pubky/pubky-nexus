...
pub fn delete_user(user_id: &str) -> Query {
    let label = "delete_user";
    let cypher = "MATCH (u:User {id: $id})
         DETACH DELETE u;";
    Query::new(label, cypher).param("id", user_id.to_string())
}
...
pub fn delete_follow(follower_id: &str, followee_id: &str) -> Query {
    let label = "delete_follow";
    let cypher = "// Important that MATCH to check if both users are in the graph
...
pub fn delete_bookmark(user_id: &str, bookmark_id: &str) -> Query {
    let label = "delete_bookmark";
    let cypher = "MATCH (u:User {id: $user_id})-[b:BOOKMARKED {id: $bookmark_id}]->(post:Post)<-[:AUTHORED]-(author:User)
...
pub fn delete_tag(user_id: &str, tag_id: &str) -> Query {
    let label = "delete_tag";
    let cypher = "MATCH (user:User {id: $user_id})-[tag:TAGGED {id: $tag_id}]->(target)
...
pub fn delete_file(owner_id: &str, file_id: &str) -> Query {
    let label = "delete_file";
    let cypher = "MATCH (f:File {id: $id, owner_id: $owner_id})
         DETACH DELETE f;";
    Query::new(label, cypher)
