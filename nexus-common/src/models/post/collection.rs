...
/// Idempotent, so any retry path may call it freely.
pub async fn sync_collected_edges(
    author_id: &str,
    post_id: &str,
    items: &[(PubkyId, String)],
) -> ModelResult<()> {
    let query = queries::put::sync_collection_items(author_id, post_id, items);
    // No row: the post is not in the graph, so there is nothing to reconcile.
