...
impl TagSearch {
    ...
    pub async fn index_post_tags_from_graph() -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        Self::add_to_global_sorted_set(global_tags_by_post_engagement(), TAG_GLOBAL_POST_ENGAGEMENT).await?;
        Ok(())
    }

    pub async fn get_by_label(label: &str, sort_by: Option<SortBy>, skip: Option<usize>, limit: Option<usize>) -> Result<Option<PostStream>, Box<dyn Error + Send + Sync>> {
        let skip = skip.unwrap_or(0);
        let limit = limit.unwrap_or(6);

        let posts_sorted_set = match sort_by {
            Some(SortBy::Engagement) => Self::try_from_index_sorted_set(&[&TAG_GLOBAL_POST_ENGAGEMENT[..], &[label]].concat(), None, None, Some(skip), Some(limit), Sorting::Descending).await?,
            // Default case always: SortBy::Timeline
            _ => Self::try_from_index_sorted_set(&[&TAG_GLOBAL_POST_TIMELINE[..], &[label]].concat(), None, None, Some(skip), Some(limit), Sorting::Descending).await?,
        };
    ...
    async fn add_to_global_sorted_set(query: Query, index_key: [&str; 4]) -> Result<(), Box<dyn Error + Send + Sync>> {
        let mut result;
        {
