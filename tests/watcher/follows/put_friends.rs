...
async fn test_homeserver_follow_friend() -> Result<()> {
    ...
    let alice_user_count = UserCounts::try_from_index_json(None, &[&alice_id])
        .await
        .unwrap()
    ...
    let bob_user_count = UserCounts::try_from_index_json(None, &[&bob_id])
        .await
        .unwrap()
        .expect("User count not found");
    ...
    // CACHE_OP: Assert if cache has been updated
    let alice_user_count = UserCounts::try_from_index_json(None, &[&bob_id])
        .await
        .unwrap()
        .expect("User count not found");
    assert_eq!(alice_user_count.followers, 1);
