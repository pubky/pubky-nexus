...
use pubky_nexus::{models::{
    ...
    pubky_app::{PostEmbed, PostKind, PubkyAppFollow, PubkyAppPost, PubkyAppUser}, user::{UserCounts, UserStream, USER_PIONEERS_KEY_PARTS},
}, RedisOps};
...
async fn test_homeserver_post_pioneer() -> Result<()> {
    ...
    let popular_user_id = test.create_user(&popular_user_keypair, &popular_user).await?;
    ...
    let pioneer_score = UserStream::check_sorted_set_member(&USER_PIONEERS_KEY_PARTS, &[&popular_user_id])
        .await
        .unwrap()
        .unwrap();
    ...
    let unknown_user_id = test.create_user(&unknown_user_keypair, &unknown_user).await?;
    ...
    let pioneer_score = UserStream::check_sorted_set_member(&USER_PIONEERS_KEY_PARTS, &[&popular_user_id])
        ...
        .unwrap()
        .unwrap();
    // TODO: update pioneers score when we follow user
    assert_eq!(pioneer_score, 0);
    ...
    let parent_uri = format!("pubky://{}/pub/pubky.app/posts/{}", popular_user_id, popular_post_id);
    ...
    let post_uri = format!("pubky://{}/pub/pubky.app/posts/{}", popular_user_id, popular_post_id);
    ...
    let popular_user_count = UserCounts::try_from_index_json(&[&popular_user_id]).await.unwrap()
        .expect("User count not found");
    assert_eq!(popular_user_count.posts, 1);

    let unknown_user_count = UserCounts::try_from_index_json(&[&unknown_user_id]).await.unwrap()
        .expect("User count not found");
    ...
    let pioneer_score = UserStream::check_sorted_set_member(&USER_PIONEERS_KEY_PARTS, &[&popular_user_id])
        ...
        .unwrap()
        .unwrap();
    // TODO: update pioneers score when posts, reply or repost
    assert_eq!(pioneer_score, 1);
