...
async fn test_homeserver_follow_friend() -> Result<()> {
    ...
    let bob_user_count = UserCounts::try_from_index_json(&[&bob_id]).await.unwrap()
    ...
    let alice_user_count = UserCounts::try_from_index_json(&[&alice_id]).await.unwrap()
        .expect("User count not found");
    ...
    let alice_user_count = UserCounts::try_from_index_json(&[&bob_id]).await.unwrap()
        .expect("User count not found");
