...
async fn test_stream_user_ids_influencers_wot_1_align() -> Result<()> {
    let query =
        format!("source=influencers&user_id={USER_ID}&timeframe=this_month&reach=wot1&limit=5");
    assert_user_ids_align(&query, "for influencers stream with reach wot1").await
}
