...
#[utoipa::path(
    ...
    tag = "Events",
    params(
        ("cursor" = u32, Query, description = "Cursor"),
        ("limit" = u32, Query, description = "Limit the number of results, (default 500, maximum 1000)")
    ),
    responses(
        (
            status = 200,
...
fn parse_query(q: &EventsQuery) -> Result<(u32, Option<u32>), Error> {
