...
#[utoipa::path(
    ...
    tag = "Events",
    params(
        ("cursor" = i64, Query, description = "Cursor"),
        ("limit" = isize, Query, description = "Limit the number of results, (default 500, maximum 1000)")
    ),
    responses(
        (
            status = 200,
...
fn parse_query(q: &EventsQuery) -> Result<(isize, Option<isize>), Error> {
