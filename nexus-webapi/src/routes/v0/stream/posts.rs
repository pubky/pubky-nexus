...
- *wot*: Requires **observer_id**. Posts from the observer and from users in the observer's Web of Trust (transitive follows, `depth` 1-3, default 2). The observer's own posts are always included, at any depth.
